...
var doneOnce = false
exports.importacionMasivaUsus = function importacionMasivaUsus(){
  if (doneOnce){
    log('@@@@@@ doneOnce @@@@@@')
    return
  }
  doneOnce = true
  var fs = require('fs');
  var lines = fs.readFileSync('/home/franz/Code/Der/py-import-xr-2/dumpNew.txt').toString().split("\n");
  //var lines = ['donpowa@gmail.com,Don,Powa,UNA_CONTRA_SEGURA']
  log('Registros=',lines.length)
  //const mails = ['sdgsdfsd1@sharklasers.com', 'sdgsdfs2d@sharklasers.com', 'sdgsdfsd3@sharklasers.com']
  lines.forEach(function(line){
    var [mail,nom,ape,pass] = line.split(',')
    if (mail == undefined || nom == undefined || ape == undefined || pass == undefined){
      log('Jumping record %s,%s,%s,%s', mail,nom,ape,pass)
      return;
    }
    var profile = {
      reference: null,
    ...
    User.register(user, profile.password, function (err, user) {
      if (err) log('Error on user %s,%s,%s: %o', mail, nom, ape, err)
      //else log('Saved user %s [%s]', user.email, user.id)
    })
  })
  log('Import done!')
}
